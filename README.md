...
- **Project proposal.** Each team submits a short written proposal on Blackboard by **Thursday, September 17** — two weeks into the term. Its purpose is to get you written feedback while there is still time to change direction. **The proposal carries 10% of your project grade**, scored pass / partial / fail on **whether you did the work — not on whether your questions turn out to be right**: a proposal whose questions we ask you to change still earns full marks if the page shows you did the thinking. A late proposal is separately penalized on the same scale as any other late project submission (see *Late submissions* below), applied to your final project grade. See `Project/PROJECT_DIRECTIVES.md` §8.1 for the scale.
...
- `lab1_python_tutorial.ipynb` — a self-paced Python/NumPy/pandas tutorial in the Session 01 folder of the course repository, **required before Session 3 (Thu Sep 10)** (\~1.5–2 hours). It is not taught live; class time from Session 03 onward assumes you have run it. One cell raises an error on purpose — that is a teaching example, not a bug.
...
| 5 | Thu Sep 17 | **Dimensionality and PCA II — nonlinear methods.** Run PCA on a real dataset, show why scaling changes the answer, and read a t-SNE or UMAP plot without over-reading it. | PCA lab on the *Universities* dataset, scaled vs. unscaled · choosing the number of components · t-SNE and UMAP as neighbourhood-preserving embeddings · why distances and cluster sizes in these plots are not interpretable | ***Project proposal due (10% of the project grade; late = standard project late penalty)*** · **Optional:** DMBA Ch. 4 |
